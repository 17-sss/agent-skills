...
- `scripts/check-native-workflow-skills.py`: Codex 특화 6개 패키지의 구조, 독립성, metadata, native capability와 source drift를 검사합니다.
...
새 스킬을 추가하거나 기존 계약을 바꿀 때는 `SKILL.md`, metadata, root catalog와 관련 테스트를 함께 갱신하고 구조 변경 후 validator를 실행합니다.
