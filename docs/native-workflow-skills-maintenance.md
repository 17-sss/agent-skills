...
5. Update only the affected `SKILL.md`, scripts, referenced contract, `agents/openai.yaml`, `metadata.json`, root catalog entry, source fingerprint, and checker inventory. Keep `INSTALLABLE_SKILL_NAMES` aligned with every directory containing a `SKILL.md`; keep `NATIVE_WORKFLOW_SKILL_NAMES` limited to native-workflow packages and use `INDEPENDENCE_VALIDATED_SKILL_NAMES` for additional standalone packages that need the same independence, sibling-reference, and state-root checks. If the minimum runtime changes, also update `CODEX_SKILL_NAMES`, `CROSS_AGENT_SKILL_NAMES`, [skill-classification.md](skill-classification.md), and TUI grouping tests. Keep `TUI_SKILL_GROUPS` aligned with the catalog when purpose grouping or experimental status changes.
...
The default invocation is offline. For every installable package listed in the checker inventory, it checks frontmatter, package-local links, executable helper bits, metadata and `agents/openai.yaml`, root catalog coverage, and TUI grouping, then runs `skill-creator`'s `quick_validate.py` when that validator and a Python interpreter with PyYAML are available. The native-workflow inventory covers the managed packages plus the experimental audio and Godot workflows; the independence inventory additionally covers `minimal`. These receive banned-runtime, hard cross-skill dependency, optional-handoff, standalone-package, and state-root checks. Source drift remains limited to the managed packages. The checker also validates guarded sibling-reference sections for `handoff-memory` and `project-chronicle`, plus retired unprefixed directories. Empty metadata reference lists are valid; every listed reference must be primary capability documentation or a standard from an allowlisted official host.
