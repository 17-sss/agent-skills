...
Read the relevant branch in [capability-routing.md](references/capability-routing.md) for a live URL, generated reference, unresolved capture-tool choice, or missing renderer. Read [visual-verdict-contract.md](references/visual-verdict-contract.md) before the first comparison and [comparison-rubric.md](references/comparison-rubric.md) before judging a render.
...
Before editing, prove equivalent rendering and capture for every required viewport and state. Match route, scroll, data, locale, theme, authentication, component state, and animation timing where relevant; report any state that cannot be reproduced. If no renderer is available, read the [no-renderer route](references/capability-routing.md#no-renderer-available): offer a minimal isolated Chromium bootstrap with its network, disk, and cache effects. Require explicit user approval unless installation was already authorized; the skill invocation alone is not installation approval. Do not modify the target repository's manifests, lockfiles, or `node_modules`. Require a disposable screenshot smoke check after an approved bootstrap. If capture remains unavailable, stop before editing with `BLOCKED`. Never claim parity from code inspection.
...
Both helpers use only the Python standard library. The semantic scorer aggregates validated native paired-image evidence; it does not inspect pixels. The PNG comparator is secondary localization evidence; its supported formats and transparency limits are in [capability-routing.md](references/capability-routing.md#scoring-capability). Keep the fixed semantic weights and default `90` threshold unless the user set another threshold before the loop.
